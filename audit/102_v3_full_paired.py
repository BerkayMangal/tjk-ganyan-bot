...
OUTPUT:
  model/trained_v3_full/      — V3 NEW v3 (cutoff 2025-05-24)
  model/trained_v3_old_full/  — V3 OLD recal (cutoff 2025-05-24, paired)
  audit/reports/phase_5_8_17_v3_full_paired.md
"""
...
OUT_OLD = os.path.join(REPO, 'model', 'trained_v3_old_full')
OUT_NEW = os.path.join(REPO, 'model', 'trained_v3_full')
REP = os.path.join(REPO, 'audit', 'reports', 'phase_5_8_17_v3_full_paired.md')
...
def main():
    ...
    logger.info(f"fc old={len(fc_old)} | new={len(fc_new)} | cutoff={CUTOFF}")
    ...
    for breed in ('arab', 'english'):
        ...
        e_old = train_one(train_df, test_df, fc_old, OUT_OLD, breed, 'V3 OLD_FULL (177)')
        e_new = train_one(train_df, test_df, fc_new, OUT_NEW, breed, 'V3 NEW_FULL (180)')
    ...
    if all_top1_pos and all_top4_pos and all_auc_pos:
        lines.append(f"**✓ V3 NEW_FULL (180) ÜSTÜN her metrikte** — swap (trained_v3 yedek + trained_v3_full → trained_v3) önerilir.\n")
    elif all_top1_pos or all_top4_pos:
        lines.append("**~ Kısmi üstünlük** — manuel inceleme.\n")
    else:
        lines.append("**✗ V3 NEW_FULL kötü** — feature seti yeniden değerlendirilmeli.\n")
