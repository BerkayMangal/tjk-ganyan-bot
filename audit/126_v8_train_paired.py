#!/usr/bin/env python3
"""V8 (235 feature) train + paired karşılaştırma vs V7 (225).
...
OUTPUT (additive, V7 dokunulmaz):
  model/trained_v8/ (V8 yeni model)
  audit/reports/phase_5_8_35_v8_paired.md
"""
...
CSV = os.path.join(REPO, 'data', 'training_v8', 'races_v8.csv')
FC_BASE = os.path.join(REPO, 'data', 'training_v7', 'feature_columns_v7.json')
FC_NEW = os.path.join(REPO, 'data', 'training_v8', 'feature_columns_v8.json')
BASE_DIR = os.path.join(REPO, 'model', 'trained_v7_225')
OUT_DIR = os.path.join(REPO, 'model', 'trained_v8')
REP = os.path.join(REPO, 'audit', 'reports', 'phase_5_8_35_v8_paired.md')
...
def main():
    ...
    logger.info(f"fc V7={len(fc_base)} | V8={len(fc_new)} | cutoff={CUTOFF}")
    ...
    for breed in ('arab', 'english'):
        ...
        eval_new[breed] = train_breed(sub, fc_new, OUT_DIR, breed, 'V8 (245)')
        ...
        logger.info(f"    V8 top1={vn['top1']*100:.2f}%  top3={vn['top3']*100:.2f}%  top4={vn['top4']*100:.2f}%")
    ...
    lines = [f"# Phase 5.8.31 — V8 (245) Paired vs V7 (225)\n",
             f"_Tarih: {datetime.utcnow().isoformat()}Z_  ·  _Cutoff: {CUTOFF}_\n\n",
             f"V8 = V7 (225) + 10 Taydex sectional sf__ features (audit/125).\n\n",
    ...
    if all_top3_pos and all_top4_pos:
        lines.append("**✓ V8 ÜSTÜN** her segmentte top3 ve top4 — V7 SHADOW yerine V8 SHADOW önerilir.\n")
    elif all_top4_pos or all_top3_pos:
        lines.append("**~ Kısmi üstünlük** — manuel inceleme + paired forward.\n")
    else:
        lines.append("**✗ V8 ZAYIF** — ix2__ feature seti yeniden değerlendirilmeli (redundant with rr__).\n")
