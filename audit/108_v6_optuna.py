#!/usr/bin/env python3
"""V6 (210) Optuna hyperparameter search — top-3 NDCG odaklı.
...
Strateji:
  ...
  - Objective: val NDCG@3 (top-3 hedefi)
...
OUTPUT:
  ...
  audit/reports/phase_5_8_23_v6_optuna.md
"""
...
OUT_DIR = os.path.join(REPO, 'model', 'trained_v6_optuna')
REP = os.path.join(REPO, 'audit', 'reports', 'phase_5_8_23_v6_optuna.md')
...
def fit_ensemble(params, X_tr, y_tr, g_tr, X_va, y_va, g_va, fin_va):
    """Fit XGB + LGBM + CB ranker with given hyperparams, return val NDCG@3 + topk."""
    ...
    ndcg3 = ndcg_at_k(p_ens, y_va, g_va, k=3)
...
def main():
    ...
    logger.info(f"\nOptuna {N_TRIALS} trials (objective: mean val NDCG@3)...")
    study.optimize(obj, n_trials=N_TRIALS, show_progress_bar=False, timeout=None)

    best = study.best_params
    logger.info(f"\nBest val NDCG@3 mean: {study.best_value:.4f}")
    ...
    # Report
    lines = ["# Phase 5.8.23 — V6 Optuna 100-trial (top-3 NDCG objective)\n",
             f"_Tarih: {datetime.utcnow().isoformat()}Z_\n\n",
             f"## Optuna\n\n",
             f"- n_trials: {len(study.trials)}\n",
             f"- best val NDCG@3 mean: **{study.best_value:.4f}**\n",
