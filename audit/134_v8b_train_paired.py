#!/usr/bin/env python3
"""V8b (235 feature) train + paired karşılaştırma vs V7 (225).
...
OUTPUT (additive, V7 dokunulmaz):
  model/trained_v8b/ (V8b yeni model)
  audit/reports/phase_5_8_42_v8b_paired.md
"""
...
CSV = os.path.join(REPO, 'data', 'training_v8b', 'races_v8b.csv')
FC_BASE = os.path.join(REPO, 'data', 'training_v7', 'feature_columns_v7.json')
FC_NEW = os.path.join(REPO, 'data', 'training_v8b', 'feature_columns_v8b.json')
BASE_DIR = os.path.join(REPO, 'model', 'trained_v7_225')
OUT_DIR = os.path.join(REPO, 'model', 'trained_v8b')
REP = os.path.join(REPO, 'audit', 'reports', 'phase_5_8_42_v8b_paired.md')
...
def main():
    ...
    logger.info(f"fc V7={len(fc_base)} | V8b={len(fc_new)} | cutoff={CUTOFF}")
    ...
    for breed in ('arab', 'english'):
        ...
        eval_new[breed] = train_breed(sub, fc_new, OUT_DIR, breed, 'V8b (235)')
        ...
        logger.info(f"    V8b top1={vn['top1']*100:.2f}%  top3={vn['top3']*100:.2f}%  top4={vn['top4']*100:.2f}%")
    ...
    lines = [f"# Phase 5.8.31 — V8b (235) Paired vs V7 (225)\n",
             f"_Tarih: {datetime.utcnow().isoformat()}Z_  ·  _Cutoff: {CUTOFF}_\n\n",
             f"V8b = V7 (225) + 10 rp__ race-pace features (audit/133).\n\n",
    ...
    if all_top3_pos and all_top4_pos:
        lines.append("**✓ V8b ÜSTÜN** her segmentte top3 ve top4 — V7 SHADOW yerine V8b SHADOW önerilir.\n")
    elif all_top4_pos or all_top3_pos:
        lines.append("**~ Kısmi üstünlük** — manuel inceleme + paired forward.\n")
    else:
        lines.append("**✗ V8b ZAYIF** — ix2__ feature seti yeniden değerlendirilmeli (redundant with rr__).\n")
