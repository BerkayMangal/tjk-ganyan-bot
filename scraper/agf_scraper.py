    ...
    for i, leg in enumerate(legs):
        ...
        for name, score, number, feat_dict in leg['horses']:
            if number in pdf_horses:
                ...
                feat_dict['weight'] = pdf_h.get('weight', 0) or feat_dict.get('weight', 0)
                feat_dict['jockey'] = pdf_h.get('jockey_name', '') or feat_dict.get('jockey', '')
                feat_dict['trainer'] = pdf_h.get('trainer_name', '') or feat_dict.get('trainer', '')
                feat_dict['form'] = pdf_h.get('form', '') or feat_dict.get('form', '')
                feat_dict['age'] = pdf_h.get('age', 0) or feat_dict.get('age', 0)
                feat_dict['age_text'] = pdf_h.get('age_text', '') or feat_dict.get('age_text', '')
                feat_dict['start_position'] = pdf_h.get('start_position', 0) or feat_dict.get('start_position', 0)
                feat_dict['handicap'] = pdf_h.get('handicap_rating', 0) or feat_dict.get('handicap', 0)
                feat_dict['equipment'] = pdf_h.get('equipment', '') or feat_dict.get('equipment', '')
                feat_dict['kgs'] = pdf_h.get('kgs', 0) or feat_dict.get('kgs', 0)
                feat_dict['last_20_score'] = pdf_h.get('last_20_score', 0) or feat_dict.get('last_20_score', 0)
                # Pedigree — from HTML scraper
                feat_dict['sire'] = pdf_h.get('sire_name', '') or feat_dict.get('sire', '')
                feat_dict['dam'] = pdf_h.get('dam_name', '') or feat_dict.get('dam', '')
                feat_dict['dam_sire'] = pdf_h.get('dam_sire_name', '') or feat_dict.get('dam_sire', '')
